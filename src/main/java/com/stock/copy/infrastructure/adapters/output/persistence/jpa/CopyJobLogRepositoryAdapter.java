...
/**
 * Adaptador JPA para ICopyJobLogRepositoryPort de stock.
 * REQ-STOCK-01.
 */
@Component
...
public class CopyJobLogRepositoryAdapter implements ICopyJobLogRepositoryPort {
    ...
    public Optional<CopyJobLog> buscarPorIdProcesoYFase(String idProceso, int fase) {
        return jpaRepository.findByIdProcesoAndFase(idProceso, fase).map(this::toDomain);
    }

    @Override
    @Transactional(readOnly = true)
    public Optional<CopyJobLog> buscarPorIdProceso(String idProceso) {
        return jpaRepository.findTopByIdProcesoOrderByFechaInicioDesc(idProceso).map(this::toDomain);
    }
    ...
    private CopyJobLogEntity toEntity(CopyJobLog log) {
        return CopyJobLogEntity.builder()
                ...
                .fase(log.getFase())
                .modulo(log.getModulo() != null ? log.getModulo() : "stock")
